...
import ScoreRate from "@/components/input/ScoreRate";
import Link from "next/link";

function Slider_product({ products }: { products: product[] }) {
  const swiperRef = useRef<SwiperRef>(null);

  const count_avaleble_average = () =>
    products?.map((i) => {
      return Math.round(
        i.count_available.reduce((i, prev) => {
          return Number(i) + Number(prev.count);
        }, 0) / i.count_available.length
      );
    });
  ...
  return (
    <div className="relative group mb-5">
      ...
      >
        {products.map((i, index) => (
          <SwiperSlide key={i._id}>
            <Link
              href={`/shop/${i._id}`}
              className="w-full h-full flex flex-col rounded-lg shadow-md hover:shadow-lg transition-all duration-300 bg-backgroundColorTheme_1 overflow-hidden"
            >
              {/* Product Image */}
              <div className="relative aspect-square w-full overflow-hidden">
                <Image
                  ...
                  src={`${process.env.NEXT_PUBLIC_SERVER}/uploads/products${i.image}`}
                  alt={i.title}
                />

                <div className="absolute bottom-2 left-1/2 transform -translate-x-1/2 bg-white/80 backdrop-blur-sm px-3 py-1 rounded-full flex gap-2">
                  {i.color.map((color) => (
                    <div
                      key={color._id}
                ...
                {/* Stock Alert */}
                {count_avaleble_average()[index] <= 8 && (
                  <div className="absolute top-2 right-2 bg-red-500 text-white text-xs px-2 py-1 rounded-full">
                    فقط {count_avaleble_average()[index]} عدد باقی مانده
                  </div>
                )}
              </div>
              ...
              <div className="p-4 flex flex-col flex-grow">
                <h3 className="font-bold line-clamp-2 mb-2">{i.title}</h3>
                <p className="text-sm line-clamp-2 mb-4">{i.low_description}</p>

                <div className="mt-auto">
                  <div className="flex items-center mb-2">
                    <ScoreRate rate={Number(i.score)} />
      ...
      <div className="swiper-button-prev absolute left-0 top-1/2 transform -translate-y-1/2 z-10 bg-backgroundColorTheme_1 shadow-ghost shadow-colorTheme rounded-[50%] w-20 h-w-20 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-300"></div>
      <div className="swiper-button-next absolute right-0 top-1/2 transform -translate-y-1/2 z-10 bg-backgroundColorTheme_1 shadow-ghost shadow-colorTheme rounded-[50%] w-20 h-w-20 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-300"></div>
    </div>
  );
}
